...
@router.post("/api/shops/{shop_id}/buy")
async def buy_from_shop(shop_id: int, buy: BuyIn, session: AsyncSession = Depends(get_session)):
    ...
    if not column:
        return JSONResponse(status_code=400, content={"message": "Novalid weight"})

    shop = await _fetch_one(session, "shop", shop_id)
    if not shop:
        return JSONResponse(status_code=404, content={"message": "Store dont serch"})

    if shop.get(column, 0) <= 0:
        return JSONResponse(status_code=400, content={"message": "Product dont have"})
